...
- (void)viewDidLoad
{
    ...
    _bankAccountText.delegate = self;
    if ([LCMyUser mine].lastBankAccount)
    {
        _bankAccountText.text = [LCMyUser mine].lastBankAccount;
    }
    else
    {
        _bankAccountText.placeholder = @"请输入银行卡号";
    }
...
- (IBAction)sendBankExchangeInfo
{
    ...
    NSString *account = _bankAccountText.text;
    
    [LCMyUser mine].lastBankAccount = _bankAccountText.text;
    
    NSLog(@"JoyYou-YMLive :: send bank exchange request with name = %@ && account = %@ && rmb = %ld", name, account, _numOfRMB);
    
    LCRequestSuccessResponseBlock successBlock = ^(NSDictionary *responseDic)
    {
        NSLog(@"JoyYou-YMLive :: requset bank exchange with response = %@", [self dicToJson:responseDic]);
        
        NSInteger code = [[responseDic objectForKey:@"stat"] integerValue];
        // 请求成功
        if (URL_REQUEST_SUCCESS == code)
        {
            NSLog(@"JoyYou-YMLive :: requset bank exchange succeeded");
        ...
        {
            NSLog(@"JoyYou-YMLive :: requset bank exchange failed");
