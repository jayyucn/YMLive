...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        int cellWidth = [reuseIdentifier isEqualToString:@"window_rank_three"] ? self.frame.size.width:SCREEN_WIDTH;
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"rank_tops_three"]];
        imageView.top = 50;
        imageView.left = 30;
        ...
        UIImage *topThree = [UIImage createContentsOfFile:@"image/liveroom/me_qb_no_3"];
        _topImg.hidden = NO;
        _topImg.image = topThree;
        _topLabelView.hidden = YES;
        _topLabel.text = @"NO.3";
        
        
        _topImg.centerX =  cellWidth/2 - topThree.size.width/6 + 1;
        _topImg.top = 2;
        _topImg.size = CGSizeMake(topThree.size.width/3, topThree.size.height/3);
        
        _faceImg.top = 15;
        _faceImg.centerX = cellWidth/2 - 7;
        _faceImg.size = CGSizeMake(67, 67);
        _faceImg.layer.cornerRadius = _faceImg.frame.size.width/2;
        _faceImg.clipsToBounds = YES;
