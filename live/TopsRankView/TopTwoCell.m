...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    ...
    if (self)
    {
        int cellWidth = [reuseIdentifier isEqualToString:@"window_rank_two"] ? self.frame.size.width:SCREEN_WIDTH;
        ...
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"rank_tops_second"]];
        imageView.top = 60;
        imageView.left = 30;
        ...
        UIImage *topTwo = [UIImage createContentsOfFile:@"image/liveroom/me_qb_no_2"];
        _topImg.hidden = NO;
        
        _topLabelView.hidden = YES;
        _topLabel.text = @"NO.2";
        
        _topImg.centerX =  cellWidth/2 - topTwo.size.width/6 + 1;
        _topImg.image = topTwo;
        _topImg.top = 3;
        _topImg.size = CGSizeMake(topTwo.size.width/3, topTwo.size.height/3);
        
        _faceImg.top = 18;
        _faceImg.centerX = cellWidth/2 - 8;
        _faceImg.size = CGSizeMake(71, 71);
        _faceImg.layer.cornerRadius = _faceImg.frame.size.width/2;
        _faceImg.clipsToBounds = YES;
