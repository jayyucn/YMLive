...
@property (nonatomic,copy) void(^completeBlock)(BOOL finished); // 新增了回调参数 finishCount， 用来记录动画结束时累加数量，将来在3秒内，还能继续累加
@end
...
//    NSLog(@"shakeNumberLabel");
    ...
    self.skLabel.text = [NSString stringWithFormat:@"X %ld",self.giftCount];
    self.skLabel.transform = CGAffineTransformMakeScale(6, 6);
...
- (void)layoutSubviews {
    ...
    _skLabel.frame = CGRectMake(CGRectGetMaxX(self.frame),-20, 50, 30);
...
- (void)setModel:(GiftModel *)model {
    _model = model;
    [_headImageView sd_setImageWithURL:[NSURL URLWithString:[NSString faceURLString:model.headUrl]] placeholderImage:[UIImage imageNamed:@"default_head"]];
//    [_giftImageView sd_setImageWithURL:[NSURL URLWithString:[NSString faceURLString:model.giftUrl]] placeholderImage:[UIImage imageNamed:@"default_head"]];
    
    ESWeakSelf;
    [[LiveGiftFile sharedLiveGiftFile] getGiftImage:model.giftId
