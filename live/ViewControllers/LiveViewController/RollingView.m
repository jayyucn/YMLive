...
@interface RollingView()<UIScrollViewDelegate>
//轮播的图片数组
...
//创建用来缓存图片的文件夹
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"RollingView"];
    BOOL isDir = NO;
...
+ (instancetype)carouselViewWithImageArray:(NSArray *)imageArray describeArray:(NSArray *)describeArray {
    RollingView *carouselView = [[self alloc] init];
...
- (void)setImageArray:(NSArray *)imageArray{
    ...
    for (int i = 0; i < imageArray.count; i++) {
        if ([imageArray[i] isKindOfClass:[UIImage class]]) {
            ...
            [_images addObject:[UIImage imageNamed:@"default_head"]];
...
- (void)downloadImages:(int)index {
    ...
    //从沙盒中取图片
    NSString *path = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"RollingView"] stringByAppendingPathComponent:[key lastPathComponent]];
    ...
    NSBlockOperation *download = [NSBlockOperation blockOperationWithBlock:^{
        ...
        //取到的data有可能不是图片
        if (image && self.images && index < self.images.count) {
