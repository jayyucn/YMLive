...
@interface LCGridItem : UIButton

@property (nonatomic, weak) LCGridMenu *menu;

@end

@implementation LCGridItem
...
@interface LCGridMenu ()

@property (nonatomic, strong) UILabel *titleLabel;
@property (nonatomic, strong) UIScrollView *contentScrollView;
@property (nonatomic, strong) LCButton *cancelButton;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _cancelButton = [LCButton buttonWithType:UIButtonTypeCustom];
...
- (void)setupWithItemTitles:(NSArray *)titles images:(NSArray *)images
{
    ...
    for (int i=0; i<titles.count; i++) {
        LCGridItem *item = [[LCGridItem alloc] initWithTitle:titles[i] image:images[i]];
        item.menu = self;
        item.tag = i;
        [item addTarget:self
                 action:@selector(tapAction:)
...
- (void)setStyle:(LCListViewStyle)style{
    _style = style;
    ...
    for (LCGridItem *item in self.items) {
