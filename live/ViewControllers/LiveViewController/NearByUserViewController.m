...
- (void)viewDidLoad
{
    ...
    self.title = ESLocalizedString(@"附近用户");
...
- (void)loadMoreData
{
    ...
    if(currentPage>=1)
        [self getNearByList:currentPage+1];
}


// 获取粉丝列表
-(void)getNearByList:(int)page
{
    ESWeakSelf;
        ...
        if(stat==200)
        {
            if(_self.isRefreshing)
            {
                ...
                else
                {
                    ...
                    self.noDataNotice.text = ESLocalizedString(@"您附近暂无玩家");
    ...
    NSDictionary *parameter = @{@"page":[NSNumber numberWithInt:page]};
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NearByUserTableCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if(cell == nil)
    {
        cell = [[NearByUserTableCell alloc]  initWithStyle:UITableViewCellStyleDefault
