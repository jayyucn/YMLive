...
@implementation UserAttentTableView
...
- (void)refreshData
{
    ...
    self.currPage = 1;
    [self requestAttentData:1];
}

- (void)loadData
{
    [self requestAttentData:self.currPage++];
}

- (void)requestAttentData:(int)page
{
    if (self.isLoading)
    {
        return;
    ...
    if (_userInfoDict && _userInfoDict[@"uid"]) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    static NSString *identifier=@"LiveAttentCell";
    ...
    cell.cellType = CELL_ATTENT_TYPE;
    if (self.datas && indexPath.row <= (self.datas.count-1)) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_showUserDetailBlock
            && self.datas && self.datas.count > 0 && indexPath.row <= (self.datas.count-1)) {
