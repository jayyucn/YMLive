...
//    UIImage* sourceImage = [UIImage imageWithContentsOfFile:[mainBundle stringByAppendingString:@"/image/animation/gift_fireworks_1"]];
    UIImage* sourceImage = [UIImage imageNamed:@"image/animation/gift_fireworks_1"];
...
//    sourceImage = [UIImage imageWithContentsOfFile:[mainBundle stringByAppendingString:@"/image/animation/gift_fireworks_2"]];
    sourceImage = [UIImage imageNamed:@"image/animation/gift_fireworks_2"];
    sourceSize = sourceImage.size;
    imagesNum = 4;
    for (int i = 0; i < imagesNum; ++i) {
        CGImageRef cgimage = CGImageCreateWithImageInRect(sourceImage.CGImage, CGRectMake(i * sourceSize.width / imagesNum, 0, sourceSize.width / imagesNum, sourceSize.height));
        //UIImage* tmp = [UIImage imageWithCGImage:cgimage];
        UIImage* tmp = [UIImage imageNamed:[NSString stringWithFormat:@"image/animation/fireworks_flower%d",i+1]];
        CGImageRelease(cgimage);
        if (tmp) {
            [imagesArr addObject:tmp];
        }
    ...
    // 3
    imagesArr = [NSMutableArray array];
//    sourceImage = [UIImage imageWithContentsOfFile:[mainBundle stringByAppendingString:@"/image/animation/gift_heart"]];
...
- (void) customInit {
//    [AnimationFireworksView initImage];
...
- (void)setMNickName:(NSString *)mNickName {
    ...
    [nameLabel sizeToFit];
    nameLabel.center = CGPointMake(self.width / 2, -65);
...
//            ESStrongSelf
//            AnimationHeartView* heartView = [[AnimationHeartView alloc] initWithFrame:CGRectMake(0, 0, const_heart_default_size, const_heart_default_size)];
//            heartView.center = CGPointMake(CGRectGetWidth(self.bounds) / 2, - 200);
//            [self addSubview:heartView];
//            [heartView playHeart];
//        });
